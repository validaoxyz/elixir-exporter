...
def parse_log_line(line):
    try:
        ...
        elif 'UPTIME:' in line:  # Updated uptime match to reflect new format
            uptime_match = re.search(r'UPTIME:\s+(\d+\.\d+)%', line)
            if uptime_match:
...
def tail_docker_logs():
    #cmd = "docker logs -f $(docker ps -q --filter ancestor=elixirprotocol/validator:main)"
